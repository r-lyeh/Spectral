...
#define BELL_ICON {7,0}

#define BACKSPACE_ICON {8,0}

#define MOUSE_ICON MAKE_ICON(9, \
    01100000, \
    01010000, \
    01001000, \
    01000100, \
...
#define CUSTOM_ICONS \
JOYSTICK_ICON, PLAY_ICON, PAUSE_ICON, TAPE_ICONL, TAPE_ICONR, \
TV_ICON, BELL_ICON, BACKSPACE_ICON, MOUSE_ICON, GUN_ICON, EJECT_ICON, REVPLAY_ICON
...
int ui_print_glyph(Tigr *ui, int ui_x, int ui_y, const rgba *colors, uint64_t ui_invert, unsigned glyph) {
    for( int i = 0; glyph >= theFont[i][0]; ++i ) {
        if( glyph == theFont[i][0] ) {
            return ui_print_glyph8x8(ui, ui_x, ui_y, colors, theFont[i][1] ^ ui_invert);
        }
    }
    return 0;
}
...
int ui_print(Tigr *ui, int ui_x, int ui_y, const rgba *colors, uint64_t ui_invert, const char *utf8) {
    ...
    while( *utf8 ) {
        ...
        if( bs ) ui_x--;
        if( !(lf+cr+bs) ) {
            ui && ui_print_glyph(ui, ui_x, ui_y += yqueue, colors, ui_invert, glyph);
            yqueue = 0;
...
int ui_mx, ui_my, ui_lmb, ui_rmb, ui_click, ui_press; // ui mouse
...
int ui_button(const char *hint8, const char *utf8) {
    ...
    while( *utf8 ) {
        ...
        if( bs ) ui_x--;
        if( !(lf+cr+bs) ) ui_x += theFontW + theFontPaddingW;
        unsigned glyph;
        utf8 = extract_utf32(utf8, &glyph);
    }
